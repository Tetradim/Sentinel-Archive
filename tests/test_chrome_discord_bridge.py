...
def test_chrome_bridge_message_records_alert_and_publishes_signal(tmp_path):
    ...
    try:
        ...
        with TestClient(app) as client:
            response = client.post(
                ...
                json={
                    ...
                    "bridge_target_id": "simulation-engine",
                    "bridge_target_name": "Simulation Engine",
                    "author_id": "mike",
            ...
            events = client.get("/api/bus/events?event_type=signal.observed").json()["events"]
        ...
        assert events[0]["payload"]["bridge_target_id"] == "simulation-engine"
...
def test_chrome_bridge_heartbeat_publishes_health_event(tmp_path):
    ...
    try:
        ...
        with TestClient(app) as client:
            ...
            events = client.get("/api/bus/events?event_type=bridge.health").json()["events"]

        assert response.status_code == 200, response.text
        assert response.json()["status"] == "healthy"
        assert health["healthy"] is True
        assert health["last_heartbeat"]["bridge_target_id"] == "simulation-engine"
        assert events[0]["payload"]["channel_url"] == "https://discord.com/channels/1/123"
    finally:
        if old_event_dir is None:
            os.environ.pop("BOT_EVENT_BUS_DIR", None)
